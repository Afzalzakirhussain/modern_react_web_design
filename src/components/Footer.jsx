                        ...
                        <div key={footerLink.key} className="flex flex-col  ss:my-0 my-4 min-w-[140px]">
                            ...
                            <ul className='list-none mt-4'>
                                {footerLink.links.map((link, index) => (
                                    <li key={link.name}
                                        className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index === footerLink.links.length - 1 ? `mb-0` : `mb-3`}`}
                                    >{link.name}</li>
                                ))}
                            </ul>
                        </div>
        ...
        <div className="flex w-full justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]">
            ...
            <div className="flex flex-row md:mt-0 mt-6">
                {socialMedia.map((media, index) => (
                    <img src={media.icon} key={media.id} alt="social.name"
